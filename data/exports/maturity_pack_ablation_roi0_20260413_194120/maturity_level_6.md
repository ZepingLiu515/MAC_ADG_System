...
本次统计摘要（由系统自动汇总）：
- 论文终态 Top1：NEEDS_REVIEW（3 篇）
- 平均每篇论文作者数：11.0；平均匹配到教师库作者数：3.8
...
- 作者级匹配：F1=97.30%（Precision=94.74% Recall=100.00%）
- 通讯作者标记：Recall=80.00%（TP=4 FN=1）
- 共一标记：Recall=50.00%（TP=1 FN=1）

本次已完成同一 5 DOI 子集的 A/B 消融，证据包如下：
- ROI 关闭（对照组）：`data/exports/maturity_pack_ablation_roi0_20260413_194120/`（本证据包）
...
对比差异的 CSV/摘要（可直接截图引用）位于 ROI 开启证据包：
- `data/exports/maturity_pack_ablation_roi1_20260413_194120/roi_ablation_delta.csv`
- `data/exports/maturity_pack_ablation_roi1_20260413_194120/roi_ablation_summary.md`
