...
- 论文级命中（是否识别到名单川大教师）：Accuracy=20.00%（TP=1 FP=0 TN=0 FN=4）
- 作者级名单匹配（真值阳性论文）：F1=27.27%（Precision=75.00% Recall=16.67%；TP=3 FP=1 FN=15）
- 通讯作者标记（真值阳性论文）：F1=33.33%（Precision=100.00% Recall=20.00%；TP=1 FN=4）
- 共一标记（真值阳性论文）：Recall=50.00%（TP=1 FN=1）
