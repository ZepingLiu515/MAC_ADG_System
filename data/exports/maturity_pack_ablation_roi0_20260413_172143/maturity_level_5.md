...
- 论文级命中（是否识别到名单川大教师）：Accuracy=0.00%（TP=0 FP=0 TN=0 FN=5）
- 作者级名单匹配（真值阳性论文）：F1=0.00%（Precision=0.00% Recall=0.00%；TP=0 FP=0 FN=18）
- 通讯作者标记（真值阳性论文）：F1=0.00%（Precision=0.00% Recall=0.00%；TP=0 FN=5）
