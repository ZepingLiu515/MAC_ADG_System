...
### 6.1 结果总览与规律发现
基于 20 篇 DOI 的运行结果，系统已能够稳定输出：论文终态（COMPLETED/NEEDS_REVIEW/SKIPPED）、作者结构化信息（单位/权益标记）、以及与四川大学教师库的身份匹配结果。

本次统计摘要（由系统自动汇总）：
- 论文终态 Top1：NEEDS_REVIEW（11 篇）
- 平均每篇论文作者数：8.76；平均匹配到教师库作者数：1.53
...
建议的消融实验协议（可复现）：
- 对照组（ROI 关闭）：
	- 设置 `PLAYWRIGHT_CAPTURE_AUTHOR_ROI=0`、`VISION_FORCE_AUTHOR_ROI=0`（仅全页截图/hover 证据）。
	- 运行同一批 DOI，导出证据包并生成 `ground_truth_eval_*.csv`。
- 实验组（ROI 开启）：
	- 设置 `PLAYWRIGHT_CAPTURE_AUTHOR_ROI=1`、`VISION_FORCE_AUTHOR_ROI=1`，并保持 `PLAYWRIGHT_DEVICE_SCALE_FACTOR=2`。
	- 运行同一批 DOI，重复导出与评测。

主指标（建议报告 2-3 个即可）：
- 通讯作者标记：Recall / Precision / F1（仅在真值阳性论文上统计）。
- 共一标记：Recall（若当前版本未稳定抽取，可先作为“待攻坚指标”记录）。
- 作者级名单匹配：Precision / Recall / F1（真值阳性论文）。

本证据包的基线（ROI=1，见 ground_truth_eval_summary.csv）：
- 作者级匹配：F1=90.91%（Precision=96.15%，Recall=86.21%）。
- 通讯作者标记：Recall=66.67%（4/6），Precision=100%（4/4）。
- 共一标记：Recall=0%（0/3）。

补充：ROI 消融（A/B）已完成（同一 5 DOI 子集，tag: 20260413_194120）
...
讨论要点（写法建议）：
- 若 ROI 开启后通讯/共一召回率上升，可归因于“微小上标/星号在全页渲染中被压缩与抗锯齿吞噬”，ROI 二次采样提升了局部像素密度与 token 可读性。
- 若共一仍然为 0，可将其作为“显式 error case”，并在后续迭代中加入站点级的 equal contribution 抽取规则与标记证据解析。
- 本次消融显示：ROI **强制**开启可改善共一标记，但可能引入漏作者与论文级漏报并增加耗时；工程上更推荐“自适应 ROI”（证据不足时启用，且 ROI 导致匹配下降时回退）。
