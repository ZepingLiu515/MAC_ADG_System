...
### 4.1 实验对象与数据规模
- 处理样本：20 篇论文 DOI（来源：doi_table.csv）
...
### 4.3 运行结果（真实统计）
- 运行成功率（流程成功，无 ERROR）：20 / 20（100%）（证据：run_times.csv）
- 平均耗时：108.0 s/篇；P90 耗时：179.2 s/篇（证据：run_times.csv / run_config.json）
- DOI 中含四川大学作者的论文数：10 / 20（比例 50.0%）
- 名单命中数（识别到教师库匹配的论文数）：10 / 20（证据：per_paper_summary.csv / ground_truth_eval_summary.csv）
- 识别到的四川大学作者提及次数：29（去重作者数 25）
- 共一标记出现的论文数：3；通讯作者标记出现的论文数：6

（补充）结构化作者条目产出覆盖率：17 / 20（85%）。其中 3 篇 DOI 在 per_paper_summary.csv 中缺失作者条目：
...
（可选）也可用 PowerShell 将输出同时写入文件：
`conda run -n doi python run_orchestrator.py --force --doi <DOI> 2>&1 | Tee-Object -FilePath data/exports/maturity_pack_20260413_135312/run_console.log`
