...
**描述**:
现在 Vision Agent 只返回整个首页的图片和文本。应该实现自适应切片：
- 从 PDF 首页识别作者区域（通常在文档上半部分）
- 将该区域裁剪成独立的图片
...
result = vision_agent.process(pdf_path)
# result 应包含:
# - result['text']: 完整文本 ✓
# - result['image_path']: 完整页面图片 ✓
...
**实现步骤**:
1. 在 `_process_pdf` 中添加自适应切片逻辑
2. 使用 PyMuPDF 的 `crop()` 方法
