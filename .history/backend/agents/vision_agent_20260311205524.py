...
class VisionAgent:
    ...
    def _call_deepseek_ocr(self, image_path):
        ...
        try:
            ...
            # 若远端 OCR endpoint 都不可用，使用 chat 接口 + vision 让文本模型识别（次优方案）
            chat_payload = {
                "model": "deepseek-chat",
                "messages": [
                    {"role": "user", "content": [
                        {"type": "image_url", "image_url": {"url": f"data:image/png;base64,{image_data}"}},
                        {"type": "text", "text": "请将图片中可见的所有文本逐行返回，仅返回纯文本，不要任何额外解释或说明。"}
                    ]}
                ],
                "max_tokens": 4096,
                "temperature": 0.0
            ...
            print("[Vision] ℹ️ 使用 chat 接口进行 OCR...")
            try:
                ...
                print(f"[Vision] ⚠️ chat 接口 OCR 失败: {e}")
                return None
