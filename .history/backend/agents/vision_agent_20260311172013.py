...
# OCR 库 - 延迟初始化（不在导入时初始化，避免启动卡住）
OCR = None  # 全局 OCR 实例，第一次使用时初始化

def _lazy_init_ocr():
    """延迟初始化 PaddleOCR（只在第一次使用时）"""
    global OCR
    if OCR is not None:
        return OCR  # 已经初始化过
    ...
    try:
        ...
        print("[Vision] 🌐 正在初始化 PaddleOCR（首次使用，可能需要下载模型...）")
        OCR = PaddleOCR(use_angle_cls=True, lang='ch')
        print("[Vision] ✅ PaddleOCR 初始化成功")
        return OCR
    except Exception as e:
        print(f"[Vision] ❌ PaddleOCR 初始化失败: {e}")
        print("[Vision] 💡 建议: pip install paddleocr --upgrade")
        return None
...
class VisionAgent:
    ...
    def _extract_text_by_ocr(self, image_path):
        """
        使用 PaddleOCR 从截图中识别文本
        延迟初始化 - 第一次调用时才初始化 OCR
        """
        # 优先尝试 DeepSeek OCR（远程服务），失败后回退到本地 PaddleOCR
        try:
            ds_text = self._call_deepseek_ocr(image_path)
            if ds_text:
                print("[Vision] ✅ DeepSeek OCR 成功")
                return ds_text
            else:
                print("[Vision] ℹ️ DeepSeek OCR 未返回文本，尝试本地 OCR")
        except Exception as e:
            print(f"[Vision] ⚠️ DeepSeek OCR 调用异常: {e}")

        # 延迟初始化 PaddleOCR
        ocr = _lazy_init_ocr()
        if ocr is None:
            print("[Vision] ⚠️  PaddleOCR 不可用")
            return None
        ...
        try:
            ...
            ocr_text = "\n".join(text_lines)
            print(f"[Vision] ✅ PaddleOCR 识别成功，提取了 {len(text_lines)} 行文本")
            return ocr_text
    ...
    def _call_deepseek_ocr(self, image_path):
        """
        调用 DeepSeek 的 OCR 接口（如果可用）。
        优先使用远端 OCR服务以避免本地模型下载和初始化问题。
        返回识别的纯文本或 None。
        """
        ...
        try:
            ...
            for url in ocr_endpoints:
                try:
                    ...
                    if resp.status_code == 200:
                        data = resp.json()
                        # 支持多种返回格式：{text: '...'} 或 {'ocr_text': '...'} 或 {'lines': [...]}
                        if isinstance(data, dict):
                            ...
                            if 'lines' in data and isinstance(data['lines'], list):
                                return "\n".join(data['lines'])
                            # 有时返回 choices/message/content（类似 chat 接口）
                            if 'choices' in data:
                                try:
                                    ...
                                    pass
                        # 若不是 200，继续尝试下一个 endpoint
                except Exception:
                    continue

            # 若远端没有 OCR endpoint 或都失败，尝试用 chat 接口让文本模型直接抽取（次优）
            chat_payload = {
                "model": "deepseek-chat",
                "messages": [
                    {"role": "user", "content": [
                        {"type": "image_url", "image_url": {"url": f"data:image/png;base64,{image_data}"}},
                        {"type": "text", "text": "请将图片中的可见文本全部直接返回，按换行分割；仅返回纯文本，不要额外说明。"}
                    ]}
                ],
                "max_tokens": 2048,
                "temperature": 0.0
            ...
            try:
                resp = requests.post(f"{DEEPSEEK_BASE_URL}/chat/completions", headers=headers, json=chat_payload, timeout=30)
                resp.raise_for_status()
                result = resp.json()
                content = result['choices'][0]['message']['content']
                # content 可能是字符串或 JSON；尽量返回字符串
